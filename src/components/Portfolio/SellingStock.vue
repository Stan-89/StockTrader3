<template>
  ...
  const buttonDisabling = computed(()=>{
    if(enteredQuantity.value == "" || enteredQuantity.value == 0 || enteredQuantity.value > props.theStock.theQuantity){
          return true;
        }
        else{
          return false;
  ...
  function sellingStock()
  {
    if(!buttonDisabling.value){
          store.sellStock({theID: props.theStock.theID, theQuantity: enteredQuantity.value});
